...
Pile::Pile(Node<Card> *card) : Stack(card) {
...
bool Pile::add(Node<Card> *card)
{
	...
	else if(this->cards->getElement()->getSuit()%2 != card->getElement()->getSuit()%2 && this->cards->getElement()->getNumber() == (card->getElement()->getNumber() + 1)) //Set a lower alternative color card
	{
		return this->Stack::add(card);
	}
	else
...
Node<Card>* Pile::remove(int index)
{
	if(index < (this->cardsNumber - this->hiddenCards)) //if the card exist
	{
		...
		this->hiddenCards = (this->cardsNumber == this->hiddenCards && this->hiddenCards > 0) ? this->hiddenCards - 1 : this->hiddenCards;
...
std::string Pile::print()
{
	if(this->cardsNumber > 0)
	{
		...
		for( ; currentNode->getPreviousElement() ; currentNode = currentNode->getPreviousElement());
