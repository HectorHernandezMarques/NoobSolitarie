...
RemainderDeck::RemainderDeck() : Stack() {
	this->hiddenCards = 0;
	this->visibleCards = 0;
}


RemainderDeck::RemainderDeck(Node<Card> *card) : Stack(card){
...
bool RemainderDeck::add(Node<Card> *card)
{
	if(this->cardsNumber)
	{
		if(this->hiddenCards)
		{
			this->visibleCards = (this->hiddenCards > GAME_MODE) ? GAME_MODE : this->hiddenCards;
			this->hiddenCards = (this->hiddenCards > GAME_MODE) ? this->hiddenCards - 3 : 0;
		}
		else
		{
			this->visibleCards = 0;
...
Node<Card>* RemainderDeck::remove(int index)
{
	...
	{
		this->visibleCards = (this->visibleCards == 1) ? 1 : this->visibleCards - 1;
...
std::string RemainderDeck::print()
{
	...
	if(this->cardsNumber)
	{
		...
		for(int i = 0 ; i < this->cardsNumber - this->hiddenCards - this->visibleCards; i++)
		{
