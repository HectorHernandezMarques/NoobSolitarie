...
template<class T> Stack<T>::Stack(Node<T> *item) :
	...
	assert(item->getPreviousElement() == nullptr);
...
template<class T> int Stack<T>::getIndex(T *item) {
	...
	Node<T> *currentItem = this->firstItem;
	int i;
	for (i = 0; currentItem->getElement() != item && i < this->itemsNumber;
			i++) {
		currentItem = currentItem->getNextElement();
	}
...
template<class T> std::string Stack<T>::toString() {
	std::string result;
